...
# source https://robotics.snowcron.com/robotics_ros2/waypoint_follower_theory.htm
...
class BasicNavigator(Node):
    ...
    def setInitialPose(self):
        try:
            t = self.tf_buffer.lookup_transform(
                ...
                'map',
                rclpy.time.Time()) 
            self.initial_pose = self.transform_to_pose(t)
    ...
    def transform_to_pose(self, transform):
        """
        Converts a tf2 transform to a geometry_msgs/msg/Pose.

        Args:
            transform: A tf2 transform object (e.g., geometry_msgs/msg/TransformStamped).

        Returns:
            A geometry_msgs/msg/Pose object representing the transform.
        """
        pose = Pose()
    ...
    def waitUntilNav2Active(self):
        print('>>> waiting for nav2_container...', end=' ')
        # self._waitForNodeToActivate('nav2_container')
        print('done.')
